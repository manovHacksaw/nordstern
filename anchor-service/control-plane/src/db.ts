...
export async function initDb() {
  // ── Operators (users) ──────────────────────────────────────────────────────
  // An operator owns MANY anchors (see tenants.owner_user_id). Registration now
  // creates the operator only — no auto-anchor (DL-005 ownership model).
  await pool.query(`
  ...
  `);
  // New anchor-factory columns (idempotent).
  await pool.query(`
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS anchor_secrets (
      ...
      role       VARCHAR(50)  NOT NULL,   -- signing | distribution | issuer
  ...
  `);
  // Defensive: nuke any legacy plaintext secret column (DL-007).
  await pool.query(`
    DO $$ BEGIN
      IF EXISTS (SELECT 1 FROM information_schema.columns
  ...
  // ── Business rules (unchanged this pass) ────────────────────────────────────
  await pool.query(`
