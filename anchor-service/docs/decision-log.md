...
## DL-010 — PSP/banking credentials live in a SecretStore, never in the DB
- **Decision:** Razorpay/Cashfree/treasury (and future banking) credentials are stored in a dedicated **SecretStore**, not in any Postgres table — not even encrypted. The application DB (`platformdb.secret_refs`) holds only a *reference*: `{ anchorId, provider, secret_provider, secret_path, key_names[], timestamps }`. All access goes through one interface — `secretStore.put/get/rotate/delete` (+ masked `describe`) — so callers never learn where a secret physically lives.
- **Backend:** AWS Secrets Manager, one secret per anchor at `nordstern/{env}/anchor/{slug}` (exactly the convention `anchor-template/infra/terraform/secrets.tf` + the ESO `externalsecret.yaml` already use). `provider` namespaces a subset of keys within that per-anchor secret; the anchor is the least-privilege / IAM boundary. Locally the **same `@aws-sdk/client-secrets-manager`** points at **LocalStack** via `SECRETS_ENDPOINT` — one code path, dev↔prod parity. A `memory` backend exists for tests and is refused in production.
- **Prod path (already scaffolded):** IRSA-scoped IAM → AWS Secrets Manager → External Secrets Operator → k8s Secret → `envFrom` into the business-server. The provisioner injects at runtime locally; ESO injects in prod. Frontend never receives values; operators add/rotate/delete only and see masked placeholders.
- **Supersedes DL-007** for PSP/banking/treasury creds (envelope-encrypted-in-`controldb` was the interim; those creds now belong in the SecretStore). Control-plane Stellar seeds remain under DL-007 for now.
- **Why chosen:** So onboarding real businesses with real keys tomorrow needs zero redesign of secret handling. Optimizes for least privilege, K8s, and production parity.
- **Tradeoffs:** Requires LocalStack (Docker) for a faithful local run; the `memory` backend covers infra-less tests. Per-provider put/get is a read-modify-write of the shared per-anchor secret (fine at operator-action concurrency). Verified via `platform/api/scripts/verify-secretstore.ts`.
