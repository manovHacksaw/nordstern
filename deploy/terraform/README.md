...
**5. Prod compose overrides** — the committed `docker-compose.platform.yml` targets the
local `db` container and plain HTTP. Add a `docker-compose.prod.yml` on the box that:
