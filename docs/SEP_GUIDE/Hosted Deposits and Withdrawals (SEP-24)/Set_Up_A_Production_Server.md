...
To switch to Stellar's public (mainnet) network, all you have to do is change the network [passphrase](https://developers.stellar.org/docs/networks#network-passphrases) (for authenticating requests) and [Horizon URL](https://horizon.stellar.org/).
...
KYC information should be linked to the session created through [Stellar Web Authentication](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep10) and, consequently, to the user, so you only need to ask the user for it once. After the first KYC flow is complete, a user shouldn't have to input the information again.
...
All SEP-9 data that was sent from the wallet is a part of the [Interactive JWT](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24/faq#how-to-use-jwts), send by the Anchor Platform
