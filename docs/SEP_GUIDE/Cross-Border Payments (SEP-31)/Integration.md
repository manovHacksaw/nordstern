---
title: "Stellar Docs"
source: "https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep31/integration"
author:
published: 2025-12-19
created: 2026-07-01
description: "Integrating with the Anchor Platform for facilitating cross-border payments involves implementing the following, at a minimum:"
tags:
  - "clippings"
---
...
- [`GET /customer`](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/get-customer) & [`PUT /customer`](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/put-customer) KYC API endpoints to request & collect customers' KYC data
- [`GET /rate`](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/get-rates) RFQ API endpoint to provide FX rates between the on & off-chain assets supported
- `GET /transactions` requests to fetch updates on the Anchor Platform's transactions' statuses (documentation coming soon)
- [`JSON-RPC`](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/platform/rpc/methods) requests to update the Anchor Platform's transactions' statuses

The following may also be required depending on your use case:

- [`DELETE /customer`](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/del-customer) if your business wants or is required to allow senders to request deletion of customer data
...
This guide does not provide an example implementation of the endpoints, but you can find more information about the request and response schemas in the [Anchor Platform API Reference](https://developers.stellar.org/docs/platforms/anchor-platform), and the sections below will expand on concepts important to understand when implementing the endpoints.
...
You can test your implementation with the [Stellar Demo Wallet](https://demo-wallet.stellar.org/) following the steps below.
...
[![demo wallet after initiating a transaction](https://developers.stellar.org/assets/images/anchor-platform-sep31-demo-wallet-widget-85fd2855c112663aa702e6da48665695.png)](https://developers.stellar.org/assets/files/anchor-platform-sep31-demo-wallet-widget-85fd2855c112663aa702e6da48665695.png)
...
Once the sending organization has registered the customers involved in the transaction, it will need to request a quote, or FX rate, from your business. The Anchor Platform requests this information from your business server using the [`GET /rate` endpoint](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/get-rates).

### Firm vs. Indicative Quotes

Requests for quotes will have a `type` parameter that is either [`indicative`](https://www.investopedia.com/terms/i/indicativequote.asp) or [`firm`](https://www.investopedia.com/terms/f/firmquote.asp). If `type=firm`, your response must include the `id` & `expires_at` date-time field and reserve the liquidity needed to fulfil this quote until the quote expires. If `type=indicative`, do not return `id` or `expires_at` fields because the rate provided will not be used in a transaction.
...
[![sep31 flow](https://developers.stellar.org/assets/images/sep31-transition-diagram-6f8dd37b304f4508346349104beddd71.png)](https://developers.stellar.org/assets/files/sep31-transition-diagram-6f8dd37b304f4508346349104beddd71.png)
...
You can create a [template](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24/integration#making-json-rpc-requests) for making a JSON-RPC requests to the Anchor Platform.

This chapter also contains information about the format of [request](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24/integration#json-rpc-request) / [response](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24/integration#json-rpc-response) and [error codes](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24/integration#error-codes) that might be returned by the Anchor Platform.
