---
title: "Stellar Docs"
source: "https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep31/configuration"
author:
...
Let's start by modifying our `stellar.toml` file created [earlier](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep1). Wallets need to know that SEP-31 functionality is supported by your business, and they also need to know all currencies you support.
...
See the [Anchor Platform KYC API specification](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/get-customer) for details on the endpoints that must be implemented on your business' server.
...
Businesses need to provide their send-side counterparts with a [Rate](https://developers.stellar.org/docs/platforms/anchor-platform/api-reference/callbacks/get-rates) API to check the exchange rates they're offering between the on-chain asset being used for settlement and the fiat asset being used to pay the recipient. If the rate is competitive, senders also need to be able to request a commitment to the rate currently being offered from business for a short period of time.
